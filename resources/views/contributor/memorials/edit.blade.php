...
<head>
    ...
    <title>Edit Memorial - Poricito</title>
...
<body class="bg-gray-50">
    <div class="min-h-screen flex flex-col">
        ...
        <div class="flex flex-1">
            ...
            <main class="flex-1 p-8">
                <a href="{{ route('contributor.memorials.show', $memorial) }}" class="text-blue-600 hover:text-blue-700 mb-4 inline-block">
                    ← Back to Memorial
                </a>

                <div class="bg-white rounded-lg shadow p-8">
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Memorial</h1>
                    <p class="text-gray-600 mb-6">Update the information and resubmit for approval.</p>

                    <form action="{{ route('contributor.memorials.update', $memorial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="bio" rows="6" 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                placeholder="Write about the person's life, achievements, and memories...">{{ old('bio', $memorial->bio) }}</textarea>
                            @error('bio')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex gap-4">
                            ...
                            <a href="{{ route('contributor.memorials.show', $memorial) }}" class="flex-1 py-3 px-6 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 font-medium text-center">
                                Cancel
                            </a>
                        </div>

                        <p class="text-sm text-gray-600 text-center">
                            Your memorial will be resubmitted for admin approval.
                        </p>
                    </form>
                </div>
            </main>
    ...
    <script>
        ...
        async function deletePhoto(photoId) {
            ...
            try {
                const response = await fetch(`/contributor/memorials/photos/${photoId}`, {
                    method: 'DELETE',
                    headers: {
